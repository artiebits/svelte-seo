...
test.describe('Testing SEO and meta tags', () => {
	...
	test('Loads the nositelinkssearchbox meta tag correctly', async ({ page }) => {
		expect(await page.locator("meta[content='nositelinkssearchbox']").getAttribute('content')).toBe(
			'nositelinkssearchbox'
		);
	});

	test('Loads the notranslate tag correctly', async ({ page }) => {
		expect(await page.locator("meta[content='notranslate']").getAttribute('content')).toBe(
			'notranslate'
		);
	});
